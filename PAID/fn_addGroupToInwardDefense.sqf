...
[{
	...
	{_x disableAI "PATH";} forEach units (P_group);

	[{
		PAID_inwardDefense_isDefenseBypassed getOrDefault [P_idName, false] && {if (typeName (P_condition) isEqualTo "STRING") then {missionNamespace getVariable (P_condition);} else {true;}}
	}, {
		...
		{_x enableAI "PATH";} forEach units (P_group);
